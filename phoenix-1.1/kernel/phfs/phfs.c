...
int phfs_open(u16 dn, char *name, u32 flags)
{
	msg_t smsg, rmsg;
	u16 l;
	u32 h;
	...
	*(u32 *)smsg.data = flags;
	memcpy(&smsg.data[sizeof(u32)], name, l);
	l += sizeof(u32);
...
int phfs_read(u16 dn, int handle, u32 *pos, u8 *buff, u32 len)
{
	msg_t smsg, rmsg;
	msg_phfsio_t *io;
	u16 hdrsz;
	...
	io = (msg_phfsio_t *)rmsg.data;  
	if ((long)io->len < 0)
		return ERR_PHFS_FILE;

	*pos = io->pos;
	l = min(io->len, msg_getlen(&rmsg) - hdrsz);
	memcpy(buff, io->data, l);
