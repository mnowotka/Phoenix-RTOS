...
static db_addr_t db_disasm_esc(db_addr_t loc, int inst, int short_addr, int size, const char *seg);

static void	db_print_address(const char *seg, int size, struct i_addr *addrp);

static db_addr_t db_read_address(db_addr_t loc, int short_addr, int regmodrm, struct i_addr *addrp);
...
void db_printsym(db_expr_t off, db_strategy_t strategy)
{
	db_printf("%x", off);
	return;
}
...
int db_read_bytes(db_addr_t addr, size_t size, char *data)
{
	char *src;
...
db_expr_t db_get_value(db_addr_t addr, register int size, boolean_t is_signed)
{
	char data[8];
...
static void db_print_address(seg, size, addrp)
	const char *	seg;
	int		size;
	struct i_addr *	addrp;
{
 ...
 * next instruction.
 */
db_addr_t db_disasm(db_addr_t loc)
{
	int	inst;
	int	size;
	int	short_addr;
...
void hal_inject(void *addr, u8 mask, u8 op)
{
	std_printf("Injecting fault at addr=%p, bit=%d, type=%s\n", addr, mask, op ? "P" : "T");

	hal_injector.orig = *(u8 *)addr;
	
	*(u8 *)addr = 0xcc;

	hal_injector.bit = mask;
	hal_injector.type = op;
	hal_injector.addr = addr;	
}
...
{
	*(u8 *)hal_injector.addr = hal_injector.orig;
	
	std_printf("Code before injection:\n");
	hal_disasm(hal_injector.addr);

	
	*(u8 *)hal_injector.addr = (hal_injector.orig & ~(1 << hal_injector.bit)) |
														 ~(hal_injector.orig & (1 << hal_injector.bit));
...
void hal_postinject(void *addr)
{
	...
	if (!hal_injector.type) {
		std_printf("Removing fault at addr=%p\n", hal_injector.addr);
		*(u8 *)hal_injector.addr = hal_injector.orig;
	}
